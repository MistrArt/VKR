...
const TourCard: React.FC<{ item: MockItem }> = ({ item }) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl shadow-blue-500/5 border border-gray-100 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/10 transition-all flex flex-col relative group">
      <button 
        onClick={handleLike}
      ...
      >
        <Heart className={`w-5 h-5 transition-colors ${isFavorite ? 'fill-red-500 text-red-500' : 'text-gray-600 hover:text-red-500'}`} />
      </button>
